...
namespace Assignment.Controllers
{
    public class ShopController : Controller
    {
        ...
        // POST Shop/AddToCart
        [HttpPost]
        public IActionResult AddToCart([FromForm] int DishId, [FromForm] int Quantity)
        {
            if (Quantity <= 0)
            {
                return BadRequest();
            ...
            var cartLine = _context.CartLines
                .Where(cl => cl.DishId == DishId && cl.CustomerId == customerId)
                .FirstOrDefault();
        ...
        public IActionResult UpdateCart([FromForm] int CartLineId, [FromForm] int Quantity)
        {
            ...
            var dish = _context.Dishes.Find(cartLine.DishId);
            var discount = cartLine.Price - (dish.Price * cartLine.Quantity);

            cartLine.Quantity = Quantity;
            cartLine.Price = Math.Max((dish.Price * Quantity) - discount, 0);
